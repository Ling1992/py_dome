...
# shell 日志
shlogdir='/Users/ling/PycharmProjects/py_dome/cache/sh/get_user/'
if [ ! -d $shlogdir ]
then
...
do
    echo $var
    var=$[ $var - 1 ]
    count=`ps aux | grep get_user.py | grep -v grep | wc -l`
    re=`ps aux | grep get_user.py | grep -v grep`
    pid=`ps aux | grep get_user.py | grep -v grep | awk '{print $2}'`
...
# # 每15分钟 执行  Python
# # */15 * * * * /bin/sh /Users/ling/PycharmProjects/py_dome/get_user.sh >/dev/null 2>&1