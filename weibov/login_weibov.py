...
class manager(object):
    ...
    sayHello = [u'@电影收录君 来一个', u'你们居然不知道@电影爬虫君 那里可以看', u'@电影爬虫君 正在免费fen想，速去围观吧', u'这个电影我早在@电影爬虫君 那里看过了', u'好哒',
                u'@电影爬虫君 那里就可以mian费观看啊', u'傻孩子，去@电影爬虫君 那里可以看的', u'科普一下：@电影爬虫君 会免费给你找汁源哒', u'还没看过的快去@电影爬虫君 吧',
                u'@电影爬虫君 可以看哦，亲测， 记得回来点个赞哦', u'大赞啊,在@电影图剪 那里找到了', u'我在@电影收录君 那里找到啦，棒棒哒', u'你们居然不知道@电影收录君 那里可以看',
                u'你也是棒，在@电影图剪 那里就有啊', u'找不到资源，谁能告诉我怎么找[二哈]']
...
class weibo(object):
    ...
    def _getrq(self):
        ...
        session.cookies = cookielib.LWPCookieJar(
            filename="{0}-{1}.txt".format(self.ns, threading.currentThread().getName()))
    ...
    def _login(self, predata):
        ...
        if predata.get('showpin', None) == 1:
            ...
            captcha = "http://login.sina.com.cn/cgi/pin.php?r={}&s=0&p={}".format(
                "".join(random.sample("123456789", 8)), predata["pcid"])
            print u'验证码', captcha, "\n"
    ...
    def comment(self, ini):
        print u'第{0}次评论， 评论内容:{1},评论人:{2}, 微博id:{3}'.format(self.count, ini['content'], self.ini['nickname'],
                                                            ini['mid'])
    ...
    def chalou(self, ini):
        ...
        print u'第{0}次评论， 评论内容:{1},评论人:{2}, 楼主：{3}, 微博id:{4}'.format(self.count, ini['content'], self.ini['nickname'],
                                                                    ini['nick'], ini['mid'])
...
if __name__ == "__main__":
    ...
    wb = weibo(users[0]['username'], users[0], workq)
