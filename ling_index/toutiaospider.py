...
class TouTiaoSpider(LingSpider):
    ...
    for_times_model_one = 3
    for_times_model_two = 10
    index = {}
    behot_time = {}
    url = {}
    ...
    def spider(self):
        ...
        # model three behot_time 每次时间戳减10*60s 获取最近两天数据
        ...
        if model[0] == 'one':
            ...
            if index is None:
                ...
                with open('cache/history_time.txt', 'w') as f:
                    f.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(behot_time - 60 * 60 * 24 * 2)))
            else:
                index -= self.sample_interval
                ...
                if response is not None:
                    ...
                    if not content or not len(content):
                        ...
                        if data["source"] == u"专题":
                            ...
                            log('content is null 跳过 !! url:{}  content:{}  dom:{}'.format(arcurl, content, dom),
                                key_str='ling')
                            continue
